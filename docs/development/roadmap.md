...
> 最終更新: 2026-09-12 / ステータス: フェーズ7(仕上げ)進行中。課金(フェーズ6)はオーナー判断待ち
...
- [x] 画像保存ポリシー(FR-12):Free即削除 / Light30日 / Pro無期限 + Storage 同期(→ [image-storage.md](./image-storage.md)。2026-09-12)
...
- [x] 期間指定(FR-19):全期間 / 年 / 月 / 任意範囲(2026-09-12)

### フェーズ6:課金・プラン
**ゴール**:プラン購入で機能が解放され、課金壁が文脈どおり出る。

- [ ] 課金基盤(RevenueCat 採用是非を検証 → [tech-stack 6章](./tech-stack.md#6-未確定要検証後続タスク))※オーナー確認
- [x] 機能ゲート結線(FR-21)・枚数上限UI(FR-22)
- [x] アップグレード画面 S-07・課金壁の発火([第4章 4.6](../requirements/04-screen-design.md#46-課金壁の発火マップまとめ))※購入ボタンは準備中表示
- 検証:Free/Light/Pro で機能の出し分け、購入・復元

### フェーズ7:Pro機能・体験の仕上げ
- [x] レシート検索(FR-13・Proゲート)/ 詳細からの編集(FR-14)/ メモ全項目(FR-09)
- [ ] 音声メモ(FR-10)/ AI学習(FR-06)/ 連続撮影(FR-03)
- [ ] オンボーディング S-01(FR-24)/ [x] Free削除バナー(FR-26)
...
- 2026-06-13: **撮影画面(S-02)を本物のカメラに差し替え**。静的モック(ダミーのレシート絵+image-picker)を expo-camera の `CameraView` ライブプレビューに変更。仕様どおり残り枚数バッジ(FR-22)・権限リクエストUI・カメラ不可環境のフォールバック(取込ボタン)・読み取り中オーバーレイを実装。app.json に expo-camera / expo-image-picker プラグイン(iOS/Android の権限文言)を追加。Webのカメラ準備直後に撮影が失敗する問題はリトライで吸収。E2E はフェイクカメラデバイスでシャッター撮影→確認画面まで検証(3本 PASS)。
  - **バグ修正**: 初期プランが `'light'` ハードコードだった問題を `DEFAULT_PLAN`(free)に修正(リリース時に全員へ有料機能が開放される穴だった)。
  - 連続撮影(FR-03)は仕様どおりフェーズ後半(phase-7-polish.md §5)。ボタンは「押せるのに動かないUIを置かない」方針で未設置。
- 2026-06-13: **ボタン総点検と Web 固有バグの一掃**(オーナー報告「❌で戻れない」を契機に全画面点検)。
  - `Alert.alert` が react-native-web で未実装(無反応)だった問題: `shared/alert.ts` の `showAlert` / `confirmAsync` に全置換。これで Web でも 詳細の削除確認・課金壁の案内・保存時の入力チェック/上限警告・認証エラー が表示される
  - 戻る/閉じるボタン: 履歴がない場合(URL直叩き・リロード)に無反応だったため `router.canGoBack()` ガード+フォールバック遷移に修正(撮影・確認・詳細)
  - 詳細画面(S-05)にヘッダー(戻るボタン)が無く行き止まりだった問題を修正
  - E2E をボタン総点検スイートに拡張(`e2e/web-buttons.spec.ts`): ナビ往復・履歴なし❌・行タップ→詳細→削除(confirmダイアログ実出現を検証)・課金壁ダイアログ・一覧検索・認証フォーム開閉/モード切替・プランチップ。**計9本 PASS**
- 2026-09-12: フェーズ7の主要タスクを実装。S-07 アップグレード画面と課金壁の結線(FR-23)、検索の Pro ゲート(FR-13)、詳細からの編集+メモ全項目(FR-14/09)、CSV期間指定(FR-19)、画像 Storage 保存と保存ポリシー(FR-12)。jest 27件・E2E 12本 PASS。
  - 購入(FR-20)は課金基盤の判断待ちのため S-07 は「準備中」表示+開発確認用の切替で代用。
  - 30日削除は閲覧時判定(サーバ cron なし)。オーナー作業: `supabase db push` で Storage マイグレーション適用。
