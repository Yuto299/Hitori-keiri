...
第5章のドラフトは以上です。以下を確認させてください。

1. **CSV形式の確定タイミング**:freee/マネフォ/弥生の正確な列定義は「実装着手時に各社の最新仕様で確定」としました(仕様が変わりやすいため)。この方針でよいですか?それとも今、各社仕様を調査して列まで確定しますか?
2. **消費税の扱い**:金額は税込合計の整数のみ保持し、消費税内訳(8%/10%・税抜)はMVPで分解しない方針です。会計ソフト取込時に税区分が必要なケースがありますが、MVPは「税込・対象外」等の固定値で割り切ってよいですか?
3. **文字コード**:汎用/freee/マネフォは UTF-8(BOM付き)、弥生のみ Shift_JIS を検討。この方針で問題ないですか?
4. **ocr_raw の保持**:OCRの生データを監査・再学習用に任意保持としました。Freeでも(画像は消すが)テキストの生抽出は残す想定です。プライバシー上の懸念はありますか?(第7章で詳述)

問題なければ第6章(OCR / AI学習の技術方針)に進みます。